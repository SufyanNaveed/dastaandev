...
class Pos_invoices_model extends CI_Model
{
    ...
    var $column_search = array('geopos_invoices.tid', 'geopos_customers.name', 'geopos_invoices.invoicedate', 'geopos_invoices.total');
    ...
    public function st_services($id){
        $this->db->select('*');
        $this->db->from('geopos_stiching_items');
    ...
    {
        $is_st = $this->input->post('st');
        
        $this->db->select('geopos_invoices.id,geopos_invoices.tid,geopos_invoices.invoicedate,geopos_invoices.invoiceduedate,geopos_invoices.total,geopos_invoices.status,geopos_invoices.notes,geopos_customers.name');
        $this->db->from($this->table);
        $this->db->where('geopos_invoices.i_class', 1);
        if ($is_st){
            $this->db->where('geopos_invoices.notes','ss');
        }
        else{
            $this->db->where('geopos_invoices.notes !=', 'ss');
        }
