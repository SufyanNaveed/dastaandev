...
class Dashboard_model extends CI_Model
{
    ...
    public function incomeChart($today, $month, $year)
    {
        ...
        $query = $this->db->query("SELECT SUM(credit) AS total,date FROM geopos_transactions WHERE ((DATE(date) BETWEEN DATE('$year-$month-01') AND '$today') AND type='Income')  $whr GROUP BY date DESC");
        return $query->result_array();
    }
    ...
    public function expenseChart($today, $month, $year)
    {
        ...
        $query = $this->db->query("SELECT SUM(debit) AS total,date FROM geopos_transactions WHERE ((DATE(date) BETWEEN DATE('$year-$month-01') AND '$today') AND type='Expense')  $whr GROUP BY date DESC");
        return $query->result_array();
    }
    ...
    public function countmonthlyChart()
    {
        ...
        $query = $this->db->query("SELECT COUNT(id) AS ttlid,SUM(total) AS total,DATE(invoicedate) as date FROM geopos_invoices WHERE (DATE(invoicedate) BETWEEN '$today' - INTERVAL 30 DAY AND '$today')  $whr GROUP BY DATE(invoicedate) DESC");
        return $query->result_array();
    }
...
FROM geopos_invoices AS i LEFT JOIN geopos_customers AS c ON i.csd=c.id $whr GROUP BY  i.csd ORDER BY  i.id  LIMIT 10");
        return $query->result_array();
