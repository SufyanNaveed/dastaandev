...
class Products_model extends CI_Model
{
    ...
    var $column_search = array('geopos_products.product_name', 'geopos_products.product_code'); //set column field database for datatable searchable
    ...
    public function quantity($pid){
        $this->db->select('geopos_product_cat.title,geopos_products.pro_than_len,SUM(geopos_products.qty) AS total_qty');
        $this->db->from($this->table);
        $this->db->join('geopos_product_cat', 'geopos_product_cat.id = geopos_products.pcat');
        $this->db->where('geopos_products.pid',$pid);
        $this->db->or_where('geopos_products.sub',$pid);
        $query = $this->db->get();
        return $query->row();
    }

    public function addnew($party_name, $product_name, $warehouse, $product_code, $color, $product_qty,$pro_than_qty, $unit, $pro_than_len, $factoryprice, $product_price, $code_type, $barcode, $taxrate, $disrate, $product_qty_alert, $product_desc, $image, $v_type, $v_stock, $v_alert, $wdate, $catid)
    {
            ...
            if ($ware_valid['loc'] == $this->aauth->get_user()->loc OR $ware_valid['loc'] == '0' OR $warehouse == 0) {
                if (strlen($barcode) > 5 AND is_numeric($barcode)) {
                    $data = array(
                        ...
                        'disrate' => $disrate,
                        'qty' => $product_qty,
                        'pro_than_qty'=>$pro_than_qty,
                        'pro_than_len'=>$pro_than_len,
    ...
    public function edit($pid, $party_name, $product_name, $warehouse, $product_code, $color, $product_qty,$pro_than_qty, $unit, $pro_than_len, $factoryprice, $product_price, $code_type, $barcode, $taxrate, $disrate, $product_qty_alert, $product_desc, $image, $v_type, $v_stock, $v_alert, $wdate, $catid)
    {
        $this->db->select('qty');
    ...
    /*genrate barcode with checkdigit*/
...
FROM geopos_products $whr");
    ...
    public function products_list($id, $term = '')
    {
        ...
        if ($this->aauth->get_user()->loc) {
            $this->db->join('geopos_warehouse', 'geopos_warehouse.id = geopos_products.warehouse');
            $this->db->where('geopos_warehouse.loc', $this->aauth->get_user()->loc);
        }  elseif(!BDATA) { $this->db->join('geopos_warehouse', 'geopos_warehouse.id = geopos_products.warehouse');
            $this->db->where('geopos_warehouse.loc', 0);  }
        if ($term) {
    ...
    public function pro_uni($pid){
        $this->db->select('geopos_units.id,geopos_units.name,geopos_units.code');
        $this->db->from('geopos_units');
        $this->db->join('geopos_products', 'geopos_products.unit = geopos_units.code');
        $this->db->where('geopos_products.pid',$pid);
        $query = $this->db->get();
        return $query->row();
    }
        ...
        foreach ($products_l as $row) {
            ...
            if ($c_qty - $qty < 0) {
                ...
                if ($c_pid) {
                    ...
                    $this->db->set('pro_than_qty', "pro_than_qty+$aQty", FALSE);
                ...
                $data['party_name_id'] = $pr['party_name_id'];
                $data['product_name'] = $pr['product_name'];
                ...
                $data['qty'] = $qty;
                $data['pro_than_qty'] = $qty/$pr['pro_than_len'];
                $data['pro_than_len'] = $pr['pro_than_len'];
