...
//search product in invoice
    public function search()
    {
        ...
        if ($name) {
            ...
            foreach ($result as $row) {
                $name = array($row['product_name'], amountExchange_s($row['product_price'], 0, $this->aauth->get_user()->loc), $row['pid'], $row['taxrate'], $row['disrate'], $row['product_des'], $row['unit'], $row['product_code'], $row['qty'], $row_num);
                array_push($out, $name);
            }
    ...
    public function puchase_search()
    {
        ...
        if ($name) {
            ...
            $result = $query->result_array();
            foreach ($result as $row) {
                $name = array($row['product_name'], amountExchange_s($row['fproduct_price'], 0, $this->aauth->get_user()->loc), $row['pid'], $row['taxrate'], $row['disrate'], $row['product_des'], $row['unit'], $row['product_code'], $row_num);
                array_push($out, $name);
            }
    ...
    public function csearch()
    {
        ...
        if ($name) {
            ...
            foreach ($result as $row) {
                ...
                echo "<li onClick=\"selectCustomer('" . $row['id'] . "','" . $row['name'] . " ','" . $row['address'] . "','" . $row['city'] . "','" . $row['phone'] . "','" . $row['email'] . "','" . $row['discount_c'] . "')\"><span>$i</span><p>" . $row['name'] . " &nbsp; &nbsp  " . $row['phone'] . "</p></li>";
                $i++;
            }
            echo '</ol>';
        }
    ...
    public function pos_search()
    {
        ...
        foreach ($result as $row) {
            ...
            $out .= '    <div class="col-3 border mb-1 "><div class="rounded">
                                 <a   id="posp' . $i . '"  class="select_pos_item btn btn-outline-light-blue round"   data-name="' . $row['product_name'] . '"  data-price=" ' . amountExchange_s($row['product_price'], 0, $this->aauth->get_user()->loc) . '"  data-tax="' . $row['taxrate'] . '"  data-discount="' . $row['disrate'] . '"   data-pcode="' . $row['product_code'] . '"   data-pid="' . $row['pid'] . '"  data-stock="' . $row['qty'] . '" data-unit="' . $row['unit'] . '" >
                                        <img class="round"
    ...
    public function v2_pos_search()
    {
        ...
        if (is_numeric($name)) {
            ...
            //    echo(substr($barcode, 0, -1));
            // $bar = " OR (geopos_products.barcode LIKE '" . (substr($barcode, 0, -1)) . "%' OR geopos_products.barcode LIKE '" . $name . "%')";
            $bar = " OR (geopos_products.barcode = '" . $name . "')";
            //  $query = "SELECT geopos_products.* FROM geopos_products $join WHERE " . $qw . " $bar AND (geopos_products.qty>0) LIMIT 16";
        }
        $query = "SELECT geopos_products.* FROM geopos_products $join WHERE " . $qw . "(UPPER(geopos_products.product_name) LIKE '%" . strtoupper($name) . "%' $bar OR geopos_products.product_code ='" . strtoupper($name) . "') AND (geopos_products.qty>0) ORDER BY geopos_products.product_name LIMIT 18";
        ...
        foreach ($result as $row) {
            ...
            $out .= '    <div class="col-2 border mb-1"  ><div class=" rounded" >
                                 <a  id="posp' . $i . '"  class="v2_select_pos_item round"   data-name="' . $row['product_name'] . '"  data-price="' . amountExchange_s($row['product_price'], 0, $this->aauth->get_user()->loc) . '"  data-tax="' . $row['taxrate'] . '"  data-discount="' . $row['disrate'] . '" data-pcode="' . $row['product_code'] . '"   data-pid="' . $row['pid'] . '" data-pcat="' . $row['pcat'] . '"  data-stock="' . $row['qty'] . '" data-unit="' . $row['unit'] . '" >
                                        <img class="round"
                                             src="' . base_url('userfiles/product/' . $row['image']) . '"  style="max-height: 100%;max-width: 100%">
