...
class Pos_invoices extends CI_Controller
{
    ...
    public function stitching(){
        ...
        $head['title'] = "Stitching Invoices";
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('invoices/st_invoices');
        $this->load->view('fixed/footer');
    }
    ...
    public function action()
    {
        ...
        $total_discount = $this->input->post('after_disc');
        ...
        if ($ptype == 4) {
            ...
            if ($this->db->insert('geopos_invoices', $data)) {
                ...
                foreach ($pid as $key => $value) {
                    ...
                    $amt = (float)$product_qty[$key];
                    if ($product_id[$key] > 0) {
                        $this->db->set('qty', "qty-$amt", FALSE);
                        $this->db->where('pid', $product_id[$key]);
                        $this->db->update('geopos_products');

                        if (($product_alert[$key] - $amt) < 0 AND $st_c == 0 AND $this->common->zero_stock()) {
...
//draft
            $p_amount = (float)$this->input->post('p_amount', true);
...
// Your Account SID and Auth Token from twilio.com/console
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $invoices) {
            ...
            // $row[] = '<a href="' . base_url("pos_invoices/view?id=$invoices->id") . '" class="btn btn-success btn-xs"><i class="fa fa-file-text"></i> ' . $this->lang->line('View') . '</a>&nbsp; <a href="' . base_url("pos_invoices/thermal_pdf?id=$invoices->id") . '&d=1" class="btn btn-info btn-xs"  title="Download"><span class="fa fa-download"></span></a>&nbsp;<a href="#" data-object-id="' . $invoices->id . '" class="btn btn-danger btn-xs delete-object"><span class="fa fa-trash"></span></a>';
            $btns = '<a href="' . base_url("pos_invoices/view?id=$invoices->id") . '" class="btn btn-success btn-xs"><i class="fa fa-file-text"></i> ' . $this->lang->line('View') . '</a>&nbsp; <a href="' . base_url("pos_invoices/thermal_pdf?id=$invoices->id") . '&d=1" class="btn btn-info btn-xs"  title="Download"><span class="fa fa-download"></span></a>';
    ...
    public function st_ajax_list()
    {
        ...
        foreach ($list as $invoices) {
            ...
            $row[] = amountExchange($invoices->total, 0, $this->aauth->get_user()->loc);
            $row[] = '<span class="st-' . $invoices->status . '">' . $this->lang->line(ucwords($invoices->status)) . '</span>';
            $row[] = '<a href="' . base_url("pos_invoices/stView?id=$invoices->id") . '" class="btn btn-success btn-xs"><i class="fa fa-file-text"></i> ' . $this->lang->line('View') . '</a>&nbsp; <a href="' . base_url("pos_invoices/thermal_pdf?id=$invoices->id") . '&d=1" class="btn btn-info btn-xs"  title="Download"><span class="fa fa-download"></span></a>&nbsp;<a href="#" data-object-id="' . $invoices->id . '" class="btn btn-danger btn-xs delete-object"><span class="fa fa-trash"></span></a>';
    ...
    public function stView(){
        ...
        if ($data['invoice']['id']) $data['products'] = $this->invocies->invoice_products($tid);
        if ($data['invoice']['id']) $data['st_servies'] = $this->invocies->st_services($tid);
        ...
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        if ($data['invoice']['id']) $this->load->view('pos/st_view', $data);
        $this->load->view('fixed/footer');
    }
    ...
    public function set_coupon()
    {
        ...
        if ($id) {
            ...
            if (($promo['active'] == 0) && ($promo['available'] > 0) && (strtotime($promo['valid']) >= strtotime(date('Y-m-d')))) {
                echo json_encode(array('status' => 'Success', 'message' => $promo['code'] . ' ' . $this->lang->line('Coupon Applied') . ' ' . $this->lang->line('Amount') . ' ' . $promo['amount'], 'amount' => $promo['amount']));
