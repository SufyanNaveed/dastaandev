    ...
    <tr>
        <td class="company">
            <?php $loc = location($invoice['loc']);
                ...
                <!-- $loc['address'] . '<br>' . $loc['city'] . ', ' . $loc['region'] . '<br>' . $loc['country'] . ' -  ' . $loc['postbox'] . '<br>' .  -->
                <?php echo
                    $this->lang->line('Phone') . ': ' . $loc['phone'] . '<br> ' . $this->lang->line('Email') . ': ' . $loc['email'];
                if ($loc['taxid']) echo '<br>' . $this->lang->line('Tax') . ' ID: ' . $loc['taxid'];
            ?>
            <?php echo '<br>'.$this->lang->line('To') . '<br>';
                echo '<strong>' . $invoice['name'] . '</strong><br>';
                if ($invoice['company']) echo $invoice['company'] . '<br>';
                //$invoice['address'] . '<br>' . $invoice['city'] . ', ' . $invoice['region'] . '<br>' . $invoice['country'] . '-' . $invoice['postbox'] . '<br>' . 
                echo  
                $this->lang->line('Phone') . ': ' . $invoice['phone'] . '<br>' . $this->lang->line('Email') . ' : ' . $invoice['email'];
...
<div id="invoice-footer" style="padding-top: 40px;"><p class="lead" style="padding-bottom: -20px;"><strong>
    <?php if($general['title'] == 'Purchase Order'){?>
    <?php echo $this->lang->line('Debit Transactions') ?>:</p></strong>
<table id="items">
